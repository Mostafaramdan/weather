...
{  
	...
	public static function base64_video($image_string,$folderName){
		if(!$image_string)return null;
			// CHECK IF FOLDER EXIST 
			$folderPath="uploads/{$folderName}/";
		if (!file_exists( $folderPath)) 
	...
	{
		if($pushNotify->$target->mute_notifications== 1){
			return true;
			\Log::info('mute_notifications :'.$pushNotify->$target->mute_notifications);
		}

		$url = 'https://fcm.googleapis.com/fcm/send';
		$body= $pushNotify->notification['content'. $pushNotify->$target->language];
		if( $pushNotify->news_id ){
			$data= objects::news($pushNotify->news);
			$type='news';
		}elseif( $pushNotify->warnings_id ){
			$data= objects::warning($pushNotify->warning);
			$type='warning';
		}else{
			$data= null;
			$type='notification';
		...
		$fields = [
			...
			"notification" => [
				"body" => strip_tags(htmlspecialchars_decode($pushNotify->notification['content'. $pushNotify->$target->language])),
				"title" =>"RasidWeather - راصد ويذر	",
				// "icon"=>         "myicon",
				"sound"=>        "notify",
	...
	public static function sendFCMV1($registration_ids,$data) 
	{
		$url = 'https://fcm.googleapis.com/fcm/send';
		...
		$result = curl_exec ( $ch );
		curl_close ( $ch );
		// if( $pushNotify->$target->id == 11)
		// 	dd($pushNotify->$target->id);
